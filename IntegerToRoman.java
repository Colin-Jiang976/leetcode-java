...
// 
//
// Related Topics Hash Table Math String 👍 8607 👎 5729
...
class Solution {
        public String intToRoman(int num) {
            ...
            // Parallel arrays acting as a lookup table for Roman numeral values and symbols.
            // They must be ordered in descending value to apply the Greedy Algorithm.
            int[] values = {1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1};
            String[] symbols = {"M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX", "V", "IV", "I"};

            // Iterate through the available denominations from largest to smallest
            for (int i = 0; i < values.length; i++) {
                // Greedily subtract the current largest value from the number
                // as long as the number is greater than or equal to that value
                while (num >= values[i]) {
                    num = num - values[i];
